...
export default function KeysChart({ targetDate, data }: KeysChartProps) {
    const bars: { label: string; height: number; today: boolean }[] = useMemo(() => {
        ...
        return [
            ...
            { label: 'NC', height: max > 0 ? (counts[5] / max) * 100 : 0, today: keysUsedToday === -1 },
        ]
    }, [data, targetDate])
    ...
    return (
        <>
            ...
            <div className="keys-chart">
                {bars.map(({ label, height, today }, index) => {
                    return (
